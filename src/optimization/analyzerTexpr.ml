...
module InterferenceReport = struct
	...
	let from_texpr e =
		...
		let rec loop e = match e.eexpr with
			...
			| TField(e1,fa) ->
				loop e1;
				set_field_read ir (field_name fa);
...
module Fusion = struct
	...
	let apply com config e =
		...
		let rec fuse acc el = match el with
			...
			| ({eexpr = TVar(v1,Some e1)} as ev) :: e2 :: el when can_be_fused v1 e1 ->
				...
				let rec replace e =
					...
					if !found then e else match e.eexpr with
						...
						| TCall({eexpr = TLocal v},el) when not (is_unbound_call_that_might_have_side_effects v el) ->
							e
						| TCall({eexpr = TField(_,FStatic(c,cf))} as e1,el) when is_pure c cf ->
			...
			| {eexpr = TBinop(OpAssign,e1,{eexpr = TBinop(op,e2,e3)})} as e :: el when is_assign_op op && target_handles_assign_ops com && Texpr.equal e1 e2 && not (Optimizer.has_side_effect e1) ->
				let rec loop e = match e.eexpr with
